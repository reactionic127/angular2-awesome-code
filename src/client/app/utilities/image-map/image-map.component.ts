...
export class ImageMapComponent implements  OnInit, OnDestroy, OnChanges {
  ...
  ngOnInit() {
    ...
    if(!this.bPreShowDamages) {
      this._eventService.registerEvent('nex_severity_event', this, (args: any) => {
        this.clickOnImage.emit({value: this.mapList[this.currentSPIndex]['data'], id: this.currentSPIndex});
      });
    } else {
  ...
  */
  loadCarImage(nCount: number = 0) {
    ...
    if(nCount > 30) {
      ...
      let that = this;
      let img = new Image();
      img.src = this.strApiUrl+ this.mapImg;
  ...
  */
  ...
  updateSelectionUI() {
    ...
    let nSection = parseInt(this.mapList[this.currentSPIndex]['data']['section']);
